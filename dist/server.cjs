...
function generateTelemetry() {
  ...
  const gpuUtil = Math.round(Math.min(99.4, Math.max(15, loadFactor * 100)) * 10) / 10;
  const vramTotal = 192;
  const vramUsed = Math.round((68.5 + gpuUtil / 100 * 54.2 + (Math.random() * 0.8 - 0.4)) * 100) / 100;
  const tempEdge = Math.round((46 + gpuUtil / 100 * 19.5 + Math.random() * 0.4) * 10) / 10;
  const tempJct = Math.round((tempEdge + 12.8) * 10) / 10;
  const tempHbm = Math.round((tempEdge + 8.4) * 10) / 10;
  const power = Math.round((240 + gpuUtil / 100 * 460 + (Math.random() * 10 - 5)) * 10) / 10;
  return {
    detected: false,
    is_emulated: true,
    ...
    gpus: [
      {
        ...
        vram_percent: Math.round(vramUsed / vramTotal * 1e3) / 10,
        gpu_utilization: gpuUtil,
        memory_bandwidth_util: Math.round(Math.min(99, gpuUtil * 0.94 + 4) * 10) / 10,
        ...
        power_watts: power,
        power_cap_watts: 750,
        fan_speed_rpm: Math.floor(3100 + gpuUtil / 100 * 1500),
        pcie_link: "PCIe 5.0 x16 (Infinity Fabric xGMI3)",
        infinity_fabric_bandwidth_gbps: 896,
        status: "HEALTHY",
        hsa_node: "/dev/kfd"
      },
      {
        ...
        vram_total_gb: vramTotal,
        vram_percent: Math.round(vramUsed * 0.82 / vramTotal * 1e3) / 10,
        gpu_utilization: Math.round(Math.max(8, gpuUtil * 0.86) * 10) / 10,
        memory_bandwidth_util: Math.round(Math.max(6, gpuUtil * 0.79) * 10) / 10,
        hbm_peak_bandwidth_tbs: 5.3,
        temp_edge_c: Math.round((tempEdge - 3.2) * 10) / 10,
        temp_junction_c: Math.round((tempJct - 3) * 10) / 10,
        temp_hbm_c: Math.round((tempHbm - 2.5) * 10) / 10,
        power_watts: Math.round(power * 0.84 * 10) / 10,
        power_cap_watts: 750,
        fan_speed_rpm: Math.floor(2900 + gpuUtil / 100 * 1300),
        pcie_link: "PCIe 5.0 x16 (Infinity Fabric xGMI3)",
        infinity_fabric_bandwidth_gbps: 896,
        status: "HEALTHY",
        hsa_node: "/dev/kfd"
...
app.post("/api/v1/triton/compile-and-run", (req, res) => {
  ...
  const peakTflops = target_arch === "gfx942" ? 1300 : target_arch === "gfx90a" ? 383 : 123;
  const peakBw = target_arch === "gfx942" ? 5300 : target_arch === "gfx90a" ? 3200 : 960;
  let efficiency = 0.86;
  ...
  const latencyMs = Math.round(latencyUs / 1e3 * 1e4) / 1e4;
  const achievedBw = Math.round(Math.min(peakBw, totalBytes / (latencyUs * 1e-6) / 1e9) * 10) / 10;
  const maxAbsDiff = dtype === "float16" ? 32e-5 : 148e-5;
...
`;
  res.json({
    ...
    metrics: {
      ...
      tflops_efficiency_percent: Math.round(achievedTflops / peakTflops * 1e3) / 10,
    ...
    verification: {
      ...
      tolerance_atol: dtype === "bfloat16" ? 0.01 : 1e-3,
      tolerance_rtol: dtype === "bfloat16" ? 0.01 : 1e-3
    },
...
function generateFallbackDiagnosis(query, kernelCtx) {
...
At \`BLOCK_M=128\` and \`BLOCK_N=128\`, each tile calculates 16,384 elements, distributing 256 MFMA operations across the 8 waves. LDS memory footprint is \`(128\xD764 + 64\xD7128) \xD7 2 bytes \xD7 2 stages = 65,536 bytes (64KB)\`, perfectly fitting within the CU LDS boundary without spill.`;
  }
  if (q.includes("error") || q.includes("divergence") || q.includes("conflict") || q.includes("bank")) {
    return `### \u{1F50D} LDS Bank Conflict & Wavefront Divergence Analysis

1. **LDS Bank Alignment**:
   AMD CDNA LDS consists of 32 memory banks spaced every 4 bytes. If stride across threads is a multiple of 128 bytes (32 banks \xD7 4 bytes), multiple lanes hit the same bank, serializing memory transactions.
...
2. **Wave64 Branch Divergence**:
  ...
  return `### \u{1F6E0}\uFE0F Mandala Infrastructure Copilot Analysis

I've reviewed your current Triton kernel configuration:
- Target: ${kernelCtx?.target_arch || "AMD Instinct MI300X (gfx942)"}
- Matrix Tile: ${kernelCtx?.block_size_m || 128}\xD7${kernelCtx?.block_size_n || 64}\xD7${kernelCtx?.block_size_k || 64}
